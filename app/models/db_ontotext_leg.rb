...
# ENV['DBAAS_COORDINATOR_ENDPOINT']
  ...
  def generate_key(db_key)
    ...
    key = new_ontotext_api_key

    if !db_key.enabled
      disable_ontotext_api_key(db_key.key['api_key'])
    end
  ...
  def delete_ontotext_repository(qds)     #Checked
    connect = ontotext_connexion(true)
    connect.delete qds.uri
  end

  # Get the size of the repository
  def get_ontotext_repository_size(se)  #Checked
    begin
  ...
  def decode_ontotext_user_id(sparql_endpoint)      #Checked
  ...
  def update_ontotext_repository_public(se)   #Checked
    begin
  ...
  # Get login status
  def get_login_status(connect)       #Checked
    resp = connect.get do |req|
  ...
  def delete_ontotext_account              #Checked
  ...
  # Methods for storage in configuration
  def ontotext_account     #Checked
    unless configuration.blank?
      ret = configuration['ontotext_account']
    end
    if ret == nil
      ret = 0
    end
  ...
  def ontotext_account=(val)   #Checked
    touch_configuration!
    configuration["ontotext_account"] = val
  end
  ...
  def self.first_or_create()     #Checked
    key = self.db_keys.where(enabled: true).first
  ...
  def register_ontotext_test_account    #Checked
    return if has_ontotext_account
